...
type PlayerTarget = {
  priority: 'must-get' | 'strong' | 'value';
  player: string;
  pos: string;
  round: string;
  why: string;
...
type DraftData = {
  ...
  fadePlayers?: FadePlayer[];
};

const fallbackPickSlots: PickSlot[] = [
...
const fallbackTargets: PlayerTarget[] = [
...
const fallbackFades: FadePlayer[] = [
...
async function getDraftData(): Promise<DraftData | null> {
  try {
    const db = getDb();
    const strategyDoc = await db.collection('analysis').doc('draftStrategy').get();
    return strategyDoc.exists ? (strategyDoc.data() as DraftData) : null;
  } catch {
    return null;
  }
}
...
export default async function DraftPage() {
  ...
  const pickSlots = data?.pickSlots || fallbackPickSlots;
  const targets = data?.playerTargets || fallbackTargets;
  const fades = data?.fadePlayers || fallbackFades;
    ...
    <div className="space-y-6">
          ...
          <table className="w-full text-sm">
            ...
            <tbody>
              {pickSlots.map((slot) => {
                ...
                return (
                  <tr key={slot.round} className={`border-b border-slate-800 ${isKey ? 'bg-green-500/5' : ''}`}>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="bg-slate-800/50 border border-green-500/30 rounded-xl p-6">
          ...
          <div className="space-y-2">
            {targets.map((target, i) => (
              <div key={`${target.player}-${i}`} className="flex items-center justify-between p-2 rounded bg-slate-800/50">
                <div className="flex items-center gap-2">
                  <span className={`w-2 h-2 rounded-full ${target.priority === 'must-get' ? 'bg-green-500' : target.priority === 'strong' ? 'bg-blue-500' : 'bg-yellow-500'}`}></span>
                  <span className="text-white text-sm font-medium">{target.player}</span>
                  <span className="text-xs px-1.5 py-0.5 rounded bg-slate-700 text-slate-400">{target.pos}</span>
                </div>
                <span className="text-xs text-slate-500">Rd {target.round}</span>
              </div>
            ))}
          </div>
        </div>
        ...
        <div className="bg-slate-800/50 border border-red-500/30 rounded-xl p-6">
          ...
          <div className="space-y-2">
            {fades.map((fade, i) => (
              <div key={`${fade.player}-${i}`} className="p-2 rounded bg-red-500/5 border border-red-500/10">
                <div className="flex items-center gap-2">
                  <span className="text-white text-sm font-medium">{fade.player}</span>
                  <span className="text-xs px-1.5 py-0.5 rounded bg-slate-700 text-slate-400">{fade.pos}</span>
                </div>
                <div className="text-xs text-red-400/70 mt-0.5">{fade.reason}</div>
              </div>
            ))}
          </div>
        </div>
